...
export default (props) => {
    ...
    const onClickEditUser = () => {
        if (deviceType == Constant.PHONE) {
            props.navigation.navigate(ScreenList.EmployeeEdit, { user: user, onCallBackEdit: getDataEdit })
    ...
    const onCLickLock = () => {
        let title = user.IsActive == true ? I18n.t('ban_co_chac_chan_muon_khoa_nhan_vien') : I18n.t('ban_co_chac_chan_muon_mo_khoa_nhan_vien')
        dialogManager.showPopupTwoButton(title, I18n.t("thong_bao"), res => {
            if (res == 1) {
                let param = { User: { ...user, IsActive: !user.IsActive } }
                new HTTPService().setPath(ApiPath.USERS).POST(param)
                    .then(res => {
                        console.log('onClickLock', res)
                        if (res && res.ResponseStatus && res.ResponseStatus.Message) {
                            ...
                            if (deviceType == Constant.PHONE) {
                                ...
                                props.route.params.onCallBack(user.IsActive == true ? 'khoa' : 'mo_khoa')
                                props.navigation.pop()
                            } else
                                props.handleSuccess(user.IsActive == true ? 'khoa' : 'mo_khoa')
                        }
                    })
                    .catch(err => console.log('onClickLock err', err))
            }
        })
    }
